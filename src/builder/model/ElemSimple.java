...
public abstract class ElemSimple extends Com5t {
    ...
    public SpcRecord spcRec = null; //спецификаци€ элемента
    ...
    public void addEvents() {
        ...
        ListenerMouse mousePressed = (evt) -> {
            if (this.area != null) {
                ...
                //≈сли клик внутри контура
                if (b == true) {
                    ++passMask[1];
                    LineSegment segm = new LineSegment(this.x1(), this.y1(), this.x2(), this.y2());
                    double coef = segm.segmentFraction(wincPress); //дол€ рассто€ни€ (в [0,0, 1,0] ) вдоль этого отрезка.
        ...
        ListenerMouse mouseDragge = (evt) -> {
            if (this.area != null) {
                ...
                //‘ильтр движухи вкл-с€ когда passMask[1] > 1 !!! 
                if (passMask[1] > 1) {
                    pointPress = evt.getPoint();
...
//        }        
    ...
    public Layout layout() {
        try {
            ...
            System.err.println("ќшибка:ElemSimple.layout() " + e);
        }
        return Layout.ANY;
    }

    //“очка редактировани€ конструкции
    @Override
    public void paint() {
        if (this.area != null) {
