...
public class Kitcalc {
    ...
    public static ArrayList<TRecord> tarifficProd(Record prjprodRec, Wincalc win, boolean normOtx, boolean numProd) {
        try {
            if (prjprodRec != null) {
                ...
                return calculate(kitList, win, 0, normOtx, numProd);
            }
        } catch (Exception e) {
    ...
    private static ArrayList<TRecord> calculate(List<Record> kit_list, Wincalc win, double discKit, boolean normOtx, boolean numProd) {
        init();
        //Цикл по комплектам
        for (Record prjkitRec : kit_list) {
            ...
            if (artiklRec != null) {
                ...
                rec.price2 = (discKit == 0) ? priceTex : priceTex - discKit * priceTex / 100; //стоимость с техн. и скидкой менеджера
                price1 += rec.price1;
                price2 += rec.price2;
                kitList.add(rec);
