...
public class DefTableModel extends DefaultTableModel implements ListenerFrame {
    ...
    public DefTableModel(JTable table, Query query, Field... columns) {
        ...
        for (int index = 0; index < columnModel.getColumnCount(); index++) {
            if (eProp.typuse.equals("99") == false && "ID".equals(table.getColumnName(index))
    ...
    public Object getValueAt(int rowIndex, int columnIndex) {
        ...
        if (columns != null) {
            ...
            Object ret = getValueAt(columnIndex, rowIndex, val); //virtual function
            if (getColumnClass(columnIndex) == Boolean.class) {
                return (ret == null || ret.equals(0) || ret.equals(false)) ? false : true;
            }
            return ret;
        }
        return null;
