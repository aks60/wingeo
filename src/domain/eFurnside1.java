...
public enum eFurnside1 implements Field {
    ...
    public static List<Record> filter(int _furniture_id) {
        if (Query.conf.equals("NET")) {
            return data().stream().filter(rec -> rec.getInt(furniture_id) == _furniture_id).collect(Collectors.toList());
        }
        Query recordList = new Query(values()).select(up, "where", furniture_id, "=", _furniture_id);
