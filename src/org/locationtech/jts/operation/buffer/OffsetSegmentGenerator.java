...
class OffsetSegmentGenerator {
    ...
    /**
     * Factor controlling how close offset segments can be to skip adding a
     * fillet or mitre. This eliminates very short fillet segments, reduces the
     * number of offset curve vertices. and improves the robustness of mitre
     * construction.
     */
    ...
    /**
     * the max error of approximation (distance) between a quad segment and the
     * true fillet curve
     */
    ...
    /**
     * The Closing Segment Length Factor controls how long "closing segments"
     * are. Closing segments are added at the middle of inside corners to ensure
     * a smoother boundary for the buffer offset curve. In some cases
     * (particularly for round joins with default-or-better quantization) the
     * closing segments can be made quite short. This substantially improves
     ...
     * closingSegFactor of 80 results in lines 1/81 of the way to the corner
     * vertex (this option is reasonable for the very common default situation
    ...
    public OffsetSegmentGenerator(Geometry geomaks, PrecisionModel precisionModel,
        ...
        /**
         * Non-round joins cause issues with short closing segments, so don't
         * use them. In any case, non-round joins only really make sense for
         * relatively small buffer distances.
         */
    ...
    /**
     * Tests whether the input has a narrow concave angle (relative to the
     * offset distance). In this case the generated offset curve will contain
     * self-intersections and heuristic closing segments. This is expected
     * behaviour in the case of Buffer curves. For pure Offset Curves, the
     * output needs to be further treated before it can be used.
     *
    ...
    private void addCollinear(boolean addStartPoint) {
        /**
         * This test could probably be done more efficiently, but the situation
         * of exact collinearity should be fairly rare.
         */
    ...
    private void addOutsideTurn(int orientation, boolean addStartPoint) {
        /**
         * Heuristic: If offset endpoints are very close together, (which
         * happens for nearly-parallel segments), use an endpoint as the single
         * offset corner vertex. This eliminates very short single-segment joins
         * and reduces the number of offset curve vertices. This also avoids
         * robustness problems with computing mitre corners for nearly-parallel
         * segments.
         */
    ...
    private void addInsideTurn(int orientation, boolean addStartPoint) {
        ...
        if (li.hasIntersection()) {
             ...
             * sure the buffer curve is continuous, fairly smooth (e.g. no sharp
             * reversals in direction) and tracks the buffer correctly around
             * the corner. The curve connects the endpoints of the segment
             * offsets to points which lie toward the centre point of the
             * corner. The joining curve will not appear in the final buffer
             * outline, since it is completely internal to the buffer polygon.
             *
             * In complex buffer cases the closing segment may cut across many
             * other segments in the generated offset curve. In order to improve
             * the performance of the noding, the closing segment should be kept
             * as short as possible. (But not too short, since that would defeat
             * its purpose). This is the purpose of the closingSegFactor
             * heuristic value.
             */
             ...
             * their endpoints, but aren't reported as such due to rounding. To
             * handle this situation appropriately, we use the following test:
            ...
            if (offset0.p1.distance(offset1.p0) < distance
                ...
                if (closingSegLengthFactor > 0) {
                    ...
                    /**
                     * This branch is not expected to be used except for testing
                     * purposes. It is equivalent to the JTS 1.9 logic for
                     * closing segments (which results in very poor performance
                     * for large buffer distances)
                     */
                    segList.addPt(s1);
                }
    ...
    /**
     * Adds a mitre join connecting two convex offset segments. The mitre is
     * beveled if it exceeds the mitre limit factor. The mitre limit is intended
     * to prevent extremely long corners occurring. If the mitre limit is very
     * small it can cause unwanted artifacts around fairly flat corners. This is
     * prevented by using a simple bevel join in this case. In other words, the
     * limit prevents the corner from getting too long, but it won't force it to
     * be very short/flat.
     *
    ...
    private void addMitreJoin(Coordinate cornerPt,
        ...
        /**
         * First try a non-beveled join. Compute the intersection point of the
         * lines determined by the offsets. Parallel or collinear lines will
         * return a null point ==> need to be beveled
         *
         * Note: This computation is unstable if the offset segments are nearly
         * collinear. However, this situation should have been eliminated
         * earlier by the check for whether the offset segment endpoints are
         * almost coincident
         */
        ...
        /**
         * In case the mitre limit is very small, try a plain bevel. Use it if
         * it's further than the limit.
         */
    ...
    /**
     * Adds a limited mitre join connecting two convex offset segments. A
     * limited mitre join is beveled at the distance determined by the mitre
     * limit factor, or as a standard bevel join, whichever is further.
     *
     * @param offset0 the first offset segment
     * @param offset1 the second offset segment
     * @param distance the offset distance
    ...
    private void addLimitedMitreJoin(
        ...
        /**
         * If the corner is very flat or the mitre limit is very small the
         * limited bevel segment may not intersect the offsets. In this case
         * just bevel the join.
         */
        addBevelJoin(offset0, offset1);
    }
    ...
    /**
     * Add points for a circular fillet around a convex corner. Adds the start
     * and end points
     *
