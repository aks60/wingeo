    ...
    private void loadingModel() {
        ...
        new DefTableModel(tab1, qKits, eKits.groups_id, eKits.name) {
            ...
            public Object getValueAt(int col, int row, Object val) {
                if (val != null && col == 0) {
                    return qCateg.find(eGroups.data(), eGroups.id, Integer.valueOf(val.toString())).getStr(eGroups.name);
                }
                return val;
            }
        };
                ...
                if (val != null) {
                    ...
                    if (field == eKitpar2.groups_id) {
                        ...
                        if (Integer.valueOf(String.valueOf(val)) < 0) {
                            return qGroups.find(eGroups.data(), eGroups.id, Integer.valueOf(val.toString())).getDev(eGroups.name, val);
                        } else {
    ...
    public void addSpecific() {
        ...
        for (Record kitdetRec : qKitdet) {
            ...
            if (kitDet.filter(mapParam, kitdetRec) == true) {
                ...
                prjkitRec.set(ePrjkit.id, Connect.genId(ePrjkit.up));
